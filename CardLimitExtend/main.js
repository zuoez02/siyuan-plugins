...
class fe extends require("siyuan").Plugin {
    async onload() {
        ...
        let confObj = {
            newCardLimitEx: (siyuan.newCardLimitEx = confData?.newCardLimitEx ?? window.siyuan.config.flashcard.newCardLimit),
            set newCardLimitEx(value) {
                siyuan.newCardLimitEx = value;
                this.writeStorage("setting.json", JSON.stringify(confObj));
            },
            get newCardLimitEx() {
                return siyuan.newCardLimitEx;
            }
        };
        const checkData = (file) => {
            return new Promise((resolve, reject) => {
                const stream = fs.createReadStream(path.join(riffDir, file));
